...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login and wait for the page to load so I can locate the tenant ID, email, and password fields.
        await page.goto("http://localhost:3000/login")
        
        # -> Fill the tenant ID, email, and password fields and submit the login form (first login attempt).
        ...
        # -> Click the 'Iniciar Sesión' button to submit the login form (second attempt) and wait for the dashboard to load.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/form/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'ID de Empresa')]").nth(0).is_visible(), "The login form should be displayed after logout so the user can sign in again"
