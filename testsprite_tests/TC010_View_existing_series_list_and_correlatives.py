...
async def run_test():
    ...
    try:
        ...
        # -> Fill the tenant ID, email and password fields and submit the login form to reach the dashboard.
        ...
        # -> Click the 'Series' link in the sidebar to open the Series management page, then verify the series table displays rows and correlative values for each row.
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'F001')]").nth(0).is_visible(), "The series table should display the existing series F001 and show a correlative value for its row."
