...
const emptyRef = (): RefForm => ({
  ...
  invoiceAmount: '', invoiceCurrency: 'PEN', paymentDate: '',
  paymentNumber: '1', paymentAmount: '', exchangeRate: '',
});
...
export default function NewRetentionPage() {
  ...
  const regimePercent: Record<string, string> = { '01': '3', '02': '6' };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center gap-4">
        <Button type="button" variant="ghost" size="sm" onClick={() => router.push('/retentions')}>
          <ArrowLeft className="h-4 w-4 mr-1" /> Retenciones
        </Button>
        <h1 className="text-2xl font-bold tracking-tight">Nueva Retención</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={form.supplierDocType} onValueChange={(v) => set('supplierDocType', v)}>
              ...
              <div>
                <Label>RUC / DNI</Label>
                <Input value={form.supplierDocNumber} onChange={e => set('supplierDocNumber', e.target.value)} required />
              </div>
            </div>
            <div>
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Datos de Retención</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Serie</Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Régimen (Catálogo 23)</Label>
                <Select value={form.regimeCode} onValueChange={(v) => {
                  if (!v) return;
                  set('regimeCode', v);
                  set('retentionPercent', regimePercent[v] || '3');
                }}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="01">01 — Tasa 3%</SelectItem>
                    <SelectItem value="02">02 — Tasa 6%</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label>% Retención</Label>
                <Input type="number" step="0.01" value={form.retentionPercent} onChange={e => set('retentionPercent', e.target.value)} />
              </div>
            </div>
            <div>
            ...
            <div key={idx} className="border rounded-lg p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <Input type="date" value={ref.paymentDate} onChange={e => setRef(idx, 'paymentDate', e.target.value)} required />
                </div>
                <div>
                  <Label className="text-xs">Nro. pago</Label>
                  <Input type="number" min="1" value={ref.paymentNumber} onChange={e => setRef(idx, 'paymentNumber', e.target.value)} />
                </div>
                <div>
                  <Label className="text-xs">Monto pagado</Label>
                  <Input type="number" step="0.01" value={ref.paymentAmount} onChange={e => setRef(idx, 'paymentAmount', e.target.value)} required />
                </div>
                <div>
      ...
      <div className="flex justify-end gap-3">
        <Button type="button" variant="outline" onClick={() => router.push('/retentions')}>Cancelar</Button>
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? 'Emitiendo...' : 'Emitir Retención'}
        </Button>
      </div>
    </form>
  );
